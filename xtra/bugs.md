...
## File Item (mosaic OR CARD)

- [SOLVED]on change progress the loader is rerendered again and again, I think that the 3 states or layers must be there from the begining
...
- [NEW] FileItem or Mosaic, downloads the image instead of video, when video present it should download the video by default. The image, by props (or both)
- [NEW] COnsider that when a big amount of files a renderred, a lazy load. DOnt know if virtualize on Dropzone or in Fikle mosaic
...
- [TO-DO] Load big images as thumbnail can cause a freeze experience (maybe adding a loader?)
